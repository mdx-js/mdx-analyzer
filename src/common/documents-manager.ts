...
export interface DocumentsManagerArgs {
  getGeneratedDocumentArgsFromDoc: (
    fileName: string,
    doc: TextDocument,
  ) => GeneratedDocumentArguments
...
export class DocumentsManager {
  ...
  private readonly generatedDocuments = new Map<
    string,
    GeneratedDocumentEntry
  >()

  private readonly externalDocuments = new Map<string, TextDocument>()

  constructor(private readonly args: DocumentsManagerArgs) {}
  ...
  private onDidOpen(e: TextDocumentChangeEvent<TextDocument>) {
    ...
    const getArgs = () =>
      this.args.getGeneratedDocumentArgsFromDoc(fileName, e.document)
    this.addOrUpdateGeneratedDocumentAndSourceMap(
      fileName,
      getArgs,
      e.document.version,
    )
  ...
  addOrUpdateGeneratedDocumentAndSourceMap(
    fileName: string,
    getArgs: () => GeneratedDocumentArguments,
    version: number,
  ) {
