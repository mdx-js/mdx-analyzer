...
export function createParseConfigHost(options: CreateParseConfigHostOptions): ts.ParseConfigHost {
...
export function createLanguageServiceHost(options: CreateLanguageServiceHostOptions) {
    const { ts, fromExtension, targetExtension, rootPath, getProxyFileOptions } = options
    ...
    function getScriptKind(fileName: string) {
        switch (upath.extname(fileName)) {
            case fromExtension: return targetExtensionToScriptKind(targetExtension);
            case ts.Extension.Js: return ts.ScriptKind.JS;
            case ts.Extension.Jsx: return ts.ScriptKind.JSX;
            case ts.Extension.Ts: return ts.ScriptKind.TS;
            case ts.Extension.Tsx: return ts.ScriptKind.TSX;
            case ts.Extension.Json: return ts.ScriptKind.JSON;
