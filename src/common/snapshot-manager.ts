...
export class SnapshotManager {
  private readonly _map = new Map<string, SnapshotEntry>()

  constructor(private readonly _options: CreateSnapshotManagerOptions) {}
  ...
  update(
    fileName: string,
    cb: (
      snap: ts.IScriptSnapshot | undefined,
      options: UpdateSnatshotOptions,
    ) => ts.IScriptSnapshot,
  ) {
    const existed = this._map.get(fileName)
    if (isDef(existed)) {
